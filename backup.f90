...
program HMMMSA
  implicit none
  integer::memstatus=0, openstatus=0, readstatus=0, sysstatus=0, system, argnum, totalseqnum, seqnum, i, j, &
           x, y, w, seqI, seqJ, a, b, p1, p2, c, ss1, ss2, k, m, n, block, L, seqL1, seqL2, len1, len2, child, &
           DPscore, Mnode
  integer(KIND=1)::iargc
  real::e, etime, time(2), bias, pseudo, shift, gapO, gapE, iden
  character(1)::signal
  ...
  if (argnum==0 .or. mod(argnum, 2)/=0) then
    write (*, *) "xHMMMSA   V. 0.0.1 Nov 2005"
    write (*, *) "Usage:"
  ...
  write (*, *) "...making initial multiple alignment"
  write (*, *)
  ...
  write (*, *) "...sequence ", trim(seqID(y)), " has been selected as the parent sequence"
  write (*, *)
  deallocate(aliscore)
  ...
  !-----build a HMM profile-----!
  allocate (HMM(3, 3, Mnode), stat=memstatus)
...
subroutine seq2drct(seqfile)
  ...
  string=' xseq2drct ' // trim(drctfile) // ' < ' // trim(seqfile) // ' >> ' // ' LOG '
  sysstatus=system(string)
...
subroutine makeMSA(seqnumber, pairs)
  ...
  character(9999), dimension(2*seqnumber), intent(inout)::pairs
